############### Alerta Telegram v3 (Filtro Address-List limit32kb) via JSON POST (v7.x) — DHCP Lease Bound ###############
:local botToken "x"
:local chatId "x"
...
:if ($leaseBound = 1) do={
    ...
    # Evaluar si el dispositivo está en el limitador buscando su IP en la Address-List "limit32kb"
    :local isBanned false
    :local listId [/ip firewall address-list find where list="limit32kb" address=$leaseIp]
    :if ([:len $listId] > 0) do={
        :set isBanned true
    }
